...
| 约束 | 说明 |
|------|------|
| **页数** | Related Work 通常 ≤ 1 页（正文共 7 页，不要在此浪费） |
| **引用格式** | 文内引用 `(Author, Year)`，`natbib` 包（不可加选项）|
| **匿名投稿** | 自引匿名化：`(Author, 2024)` → 替换为描述性文本 "Previous work [X]..."。不可出现 "In our previous work..." |
| **参考文献字号** | 由 `aaai2027.sty` 自动设置，手动调整会被退稿 |
